...
def open_state(config: dict[str, Any]) -> SyncState:
    db_path = PROJECT_ROOT / config.get("state_path", "var/ragflow-sync-state.db")
    if db_path.suffix != ".db":
        db_path = db_path.with_suffix(".db")
    return SyncState(db_path)
...
def verify_remote(
    ...
    """Query RAGFlow for every known document, update remote_status in DB."""
    ...
    for doc in docs:
        ...
        run = doc.get("run", "")
        status = RUN_STATUS_MAP.get(run, run.lower() if run else "pending")
        msg = ""
        if run == "FAIL":
            msg_lines = (doc.get("progress_msg") or "").strip().split("\n")
            msg = msg_lines[-1][:200] if msg_lines else ""
        updates.append((doc_id, status, msg))
    if updates:
        state.update_remote_batch(dataset.source_dir, updates)
    missing = state.mark_remote_missing(dataset.source_dir, live_doc_ids)
    return {"verified": len(updates), "missing": missing}
    ...
    try:
        for dataset in pick_datasets(config, args.dataset):
            ...
            for row in state.files_by_source(dataset.source_dir):
                if row["path"] not in live_keys and row.get("document_id"):
...
def cmd_upload(args: argparse.Namespace) -> int:
                ...
                if item and item.get("document_id") and item.get("sha256") != digest:
                    ...
                    try:
                        ...
                        pending_records.append({
                            ...
                            "local_status": "deleted_before_replace",
                            "remote_status": "",
                            "remote_msg": "",
                            "last_verified": "",
                        })
    ...
    if args.parse and not args.dry_run:
        ...
        for dataset_id, ids in uploaded_for_parse.items():
            ...
            print(f"parse {ds.source_dir}: {len(ids)} documents")
            parse_docs(base_url, api_key, ds, ids)
    return 0
